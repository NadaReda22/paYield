...
class VendorProductController extends Controller
{
    public function index(): View
    {
        ...
        $products = Product::with('vendor')->whereHas('vendor',function($query) use ($id){
          $query->where('id',$id);
        })->latest()->get();

        return view('Vendor.product.all_product',compact('products'));
    }
    ...
    public function create() :View
    {
        ...
        return view('vendor.product.add_product', compact(['categories', 'brands','multiImg']));
    }
...
public function edit($id): View
{
    ...
    // $activeVendor = User::where('role', 'vendor')->where('status', 'active')->get();
    // dd($product); 
    return view('vendor.product.edit_product', compact(['product', 'categories', 'brands','multiImgs']));
}
 ...
 //Product Delete
    public function destroy($id)
    {
        $product = Product::findOrFail($id);
            ...
            // First, delete related images in multi_imgs
    ...
    public function ProductrDetails($id)
    {   
        $product=Product::findOrFail($id);
        ...
        $vendor=$product->vendor;
          ...
          $product_size = $product->product_size ? explode(',', $product->product_size) : [];
          $product_color = $product->product_color ? explode(',', $product->product_color) : [];
          $product_tag = $product->product_tags ? explode(',', $product->product_tags) : [];
...
//    dd($multiImage, $brands, $product, $vendor, $product_size, $product_color, $product_tag, $relatedProduct);


        return view('frontend.product.product_details',compact(['multiImage','brands','product','vendor',
        'product_size','product_color','product_tag','relatedProduct']));
    }
}
