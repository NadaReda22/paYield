    ...
    <div class="page-content">
        ...
        <div class="card">
            <div class="card-body p-4">
                ...
                <form id="myForm" method="post" action="/admin/product/update/{{$product->id}}">
                    @csrf
                    ...
                    <div class="form-body mt-4">
                        <div class="row">
                          ...
                          <select name="brands[]" class="form-select" id="inputProductType" multiple>
                            @php
                                $selectedBrands = (array) old('brands', $product->brands->pluck('id')->toArray());
                            @endphp
                            ...
                            @endforeach
                                        ...
                                        <div class="col-12">
                                            ...
                                            <select name="user_id" class="form-select" id="inputCollection">
                                                <option>Select the vendor name</option>
                                              @foreach ($activeVendor as $vendor) 
                                                    <option value="{{$vendor->id }}" {{$vendor->id ==old('user_id',$product->user_id) ? 'selected' : ''}}>
                                                    {{$vendor->name}}
                                                   </option>
                                                @endforeach
                                            </select>
        ...
        <div class="card">
            ...
            <div class="card-body">
                <form method="post" enctype="multipart/form-data" action="/admin/product/thumbnail/update/{{$product->id}}">{{--update image only why??--}}
                    @csrf
                    <input type="hidden" name="id" value="{{$product->id}}" />
                ...
                <div class="card-body">
                    <table class="table mb-0 table-striped">
                        ...
                        <tbody>
                            ...
                            <form method="post" action="/admin/product/multiimg/update" enctype="multipart/form-data">
                                @csrf

                                @foreach ($multiImg as $count => $img)
                                    <tr>
                                        <th scope="row">{{$count +1}}</th>
                                        <td>

                                            <img id="" src="{{ asset($img->image) }}" style="width:70; height: 70px; border : 1px solid gainsboro; border-radius : 5px">
                                        </td>
                                        ...
                                        <td>
                                            ...
                                            <a href="/admin/product/multiimg/delete/{{$img->id}}" class="btn btn-danger"
                                                id="delete"> Delete </a>
                                        </td>
                                    </tr>
                                @endforeach
